# GameCollection API – RepositoryAndServiceLayer i Docker

Byg og kør fra repositoryets rod på denne branch:

```powershell
docker build -t gamecollectionapi:repository .
docker run --rm -p 5000:80 gamecollectionapi:repository
```
...
```powershell
docker run --rm -p 5000:80 -e Repository__Provider=Sqlite -v gamecollection-sqlite:/data gamecollectionapi:repository
```

Brug denne branchs image-navn i stedet for gamecollectionapi:repository. /data er skrivbar for containerens app-bruger.
